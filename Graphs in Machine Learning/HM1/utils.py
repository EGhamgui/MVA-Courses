...
def plot_clusters(X, Y, fignum='data', ax = None , title = '', show=False):
    """
...
def plot_edges_and_points(X, Y, W, ax = None, title=''):

    ax = ax or plt.gca()
    colors=['go-', 'ro-', 'co-','yo-', 'ko-', 'mo-']
    n=len(X)
...
def plot_clustering_result(X, Y, W, spectral_labels, kmeans_labels, normalized_switch=0):
    ...
    plt.clf()
    plt.subplot(1, 3, 1)
    plot_edges_and_points(X, Y, W, None,'ground truth')
    plt.subplot(1, 3, 2)
    if normalized_switch:
        plot_edges_and_points(X, spectral_labels, W,None, 'unnormalized laplacian')
    else:
        plot_edges_and_points(X, spectral_labels, W,None, 'spectral clustering')
    plt.subplot(1, 3, 3)
    if normalized_switch:
        plot_edges_and_points(X, kmeans_labels, W,None, 'normalized laplacian')
    else:
        plot_edges_and_points(X, kmeans_labels, W,None, 'k-means')
    plt.show()
